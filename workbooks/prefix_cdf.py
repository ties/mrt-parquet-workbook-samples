...
def _(df, pl, plt):
    ...
    peer_counts = df.group_by(["peer_ip", "peer_asn", "proto"]).agg(
        pl.col("cnt").sum().alias("prefix_count")
    )
    ...
    for proto in protocols:
        ...
        proto_data = (
            peer_counts.filter(pl.col("proto") == proto)
            .select("prefix_count")
            .sort("prefix_count")
            .to_series()
            .to_list()
...
@app.cell
def _(duckdb):
    df_1min = duckdb.query(
        "select count(*) num_updates, time_bucket(interval '5 minutes', ts) time_window from '~/src/tmp/parquet/updates/**/*.parquet' where day='26' group by all order by time_window asc;"
    ).df()
    return (df_1min,)
